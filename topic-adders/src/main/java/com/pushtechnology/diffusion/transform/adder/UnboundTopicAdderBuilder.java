 ...
 */
@Deprecated
public interface UnboundTopicAdderBuilder<S, T, U extends TopicAdder<T>> extends TopicAdderBuilder<S, T> {
    @Override
     ...
     */
    BoundTopicAdderBuilder<S, T, U> bind(Session session);

    /**
     * Create the adder.
     *
     * @param session the session to create the adder for
     */
