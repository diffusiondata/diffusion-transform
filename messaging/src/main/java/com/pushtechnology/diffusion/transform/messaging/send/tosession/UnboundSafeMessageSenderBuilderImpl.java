...
public final class UnboundSafeMessageSenderBuilderImpl<V> implements UnboundSafeMessageSenderBuilder<V> {
    ...
    public BoundSafeMessageSenderBuilder<V> bind(Session session) {
        return new BoundSafeMessageSenderBuilderImpl<>(session.feature(MessagingControl.class), transformer);
    }

    @Override
    public SafeMessageSender<V> build(Session session) {
        return new SafeMessageSenderImpl<>(session.feature(MessagingControl.class), transformer);
    }
}
