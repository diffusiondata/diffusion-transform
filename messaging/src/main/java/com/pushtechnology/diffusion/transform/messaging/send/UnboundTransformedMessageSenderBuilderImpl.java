...
/*package*/ final class UnboundTransformedMessageSenderBuilderImpl<V> implements
    ...
    public BoundTransformedMessageSenderBuilder<V> bind(Session session) {
        return new BoundTransformedMessageSenderBuilderImpl<>(session, transformer);
    }

    @Override
    public MessageToSessionSender<V> buildToSessionSender(Session session) {
        return new MessageToSessionSenderImpl<>(session.feature(MessagingControl.class), transformer);
    }

    @Override
    public MessageToHandlerSender<V> buildToHandlerSender(Session session) {
